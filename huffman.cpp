...
/*压缩文件*/
void compressFile(const char* sourceFilename,const char* geneFilename) {
    NodeList list;
...
/*解压文件*/
void uncompressFile(const char* geneFilename,const char* backFilename) {                                    //从树信息文件读取的所有结点个数
    ...
    if (length == 0) { printf("请输入正确的压缩包路径"); fclose(fw); fclose(fr); return; }
